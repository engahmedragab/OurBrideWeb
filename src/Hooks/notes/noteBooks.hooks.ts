...
import {
  ...
  syncNoteBook,
  getNoteBook,
} from '@/services/api/noteBooksApi'
...
const normalizeQuerySafe = (
  query?: NoteBooksQuery
): NoteBooksQuery | undefined => {
...
const invalidateNoteBookQueries = (
  ...
  if (query)
    queryClient.invalidateQueries({ queryKey: noteBookKeys.book(query) })
}

export const useNoteBook = (
  query?: NoteBooksQuery,
  options?: Omit<
    UseQueryOptions<NoteBookResponse | null, Error>,
    'queryKey' | 'queryFn'
  >
) => {
...
export const useInitNoteBooks = (
  options?: Omit<
    UseMutationOptions<
      void,
      Error,
      { eventId?: number; clientId?: string; userType?: any },
      unknown
    >,
    'mutationFn'
  >
  ...
  >({
    mutationFn: async params => {
      ...
      if (typeof safe.clientId === 'string' && !safe.clientId.trim())
    ...
    onSuccess: (_data, variables) => {
      invalidateNoteBookQueries(
        queryClient,
        normalizeQuerySafe(variables as any)
      )
    },
    ...options,
  })
...
export const useNoteSyncMutation = (
  options?: Omit<
    UseMutationOptions<
      void,
      Error,
      NoteBookRequest & { query?: NoteBooksQuery },
      unknown
    >,
    'mutationFn'
  >
  ...
  >({
    ...
    onSuccess: (_data, variables) => {
      invalidateNoteBookQueries(
        queryClient,
        normalizeQuerySafe(variables.query)
      )
    },
    ...options,
  })
