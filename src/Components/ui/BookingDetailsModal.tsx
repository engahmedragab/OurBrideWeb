...
import { X, User, Phone, Tag, Diamond, Gift, ChevronDown, Calendar, Star } from 'lucide-react'
...
const mockPackages: Package[] = [
  { id: '1', title: 'Package Title', price: 4500, description: 'Basic package' },
  { id: '2', title: 'Package Title', price: 6000, description: 'Standard package' },
  { id: '3', title: 'Package Title', price: 8000, description: 'Premium package' },
]
...
export const BookingDetailsModal = ({
  ...
  const selectedUpgradesData = packageUpgrades.filter(u => formData.selectedUpgrades.includes(u.id))
  
  const subtotal = (selectedPackageData?.price || 0) + selectedUpgradesData.reduce((sum, u) => sum + u.price, 0)
  const taxes = 120
    ...
    >
      <div className="overflow-y-auto max-h-[calc(90vh-80px)]">
        <div className="flex flex-col lg:flex-row bg-white">
          ...
          <div className="mb-6">
            <h3 className="text-16 font-semibold text-gray-900 mb-4">Personal Information</h3>
          ...
          {/* Available Branches */}
          <div className="mb-6">
            <h3 className="text-16 font-semibold text-gray-900 mb-4">Available Branches</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {branches.map((branch) => (
                <button
                  key={branch.id}
                  type="button"
                  onClick={() => handleInputChange('selectedBranch', branch.id)}
                  className={cn(
          ...
          {/* Select Your Package */}
          <div className="mb-6">
            <h3 className="text-16 font-semibold text-gray-900 mb-4">Select Your Package</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {packages.map((pkg) => (
                ...
                >
                  <div className="text-14 font-semibold text-gray-900 mb-1">{pkg.title}</div>
                  <div className="text-14 text-gray-600">
                    Price | {pkg.price.toLocaleString()} {service.price.currency.toUpperCase()}
                  </div>
                </button>
              ))}
            </div>
          ...
          {/* Package Details */}
          <div className="mb-6">
            <h3 className="text-16 font-semibold text-gray-900 mb-4">Package Details</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {packageUpgrades.map((upgrade) => (
                ...
                >
                  <div className="text-14 font-semibold text-gray-900 mb-1">{upgrade.title}</div>
                  <div className="text-14 text-gray-600">
                    Price | {upgrade.price.toLocaleString()} {service.price.currency.toUpperCase()}
                  </div>
                </button>
              ))}
            </div>
          ...
          {/* Available Times */}
          <div className="mb-6">
            <h3 className="text-16 font-semibold text-gray-900 mb-4">Available Times</h3>
            <div className="mb-4">
              <DatePicker
            ...
            {errors.selectedTime && (
              <p className="text-12 text-red-500 mt-2">{errors.selectedTime}</p>
            )}
          </div>
        </div>
          ...
          <div className="w-full lg:w-[40%] lg:flex-shrink-0 p-4 md:p-6 bg-gray-50">
            <h3 className="text-20 font-semibold text-gray-900 mb-6">Booking Summary</h3>
          ...
          <div className="bg-white rounded-lg p-4 mb-6 border border-gray-200">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h4 className="text-16 font-semibold text-gray-900 mb-2">{service.title}</h4>
                ...
                <p className="text-14 text-gray-600">Provider: {service.provider.name}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2">
              <div className="flex-1 flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-gray-300">
                ...
                <span className="text-14 text-gray-900">Diamonds: 250 Points</span>
              </div>
              <Button
                variant="default"
                className="h-10 px-4 rounded-lg bg-brand-500 hover:bg-brand-600 text-white"
                onClick={() => handleInputChange('useDiamonds', !formData.useDiamonds)}
              >
                Redeem
              </Button>
            </div>
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2">
              <div className="flex-1 flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-gray-300">
                ...
                <span className="text-14 text-gray-900">Gifts Cash: 500 {service.price.currency.toUpperCase()}</span>
              </div>
              <Button
                variant="default"
                className="h-10 px-4 rounded-lg bg-brand-500 hover:bg-brand-600 text-white"
                onClick={() => handleInputChange('useGiftsCash', !formData.useGiftsCash)}
              >
                Redeem
              </Button>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-14 font-semibold text-gray-900">
                  {subtotal.toLocaleString()} {service.price.currency.toUpperCase()}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-14 text-gray-600">Taxes & Fees</span>
                <span className="text-14 font-semibold text-gray-900">
                  {taxes.toLocaleString()} {service.price.currency.toUpperCase()}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-14 text-gray-600">Delivery Fee</span>
                <span className="text-14 font-semibold text-gray-900">
                  {deliveryFee.toLocaleString()} {service.price.currency.toUpperCase()}
                </span>
              </div>
              <div className="pt-3 border-t border-gray-200 flex justify-between items-center">
                ...
                <span className="text-16 font-semibold text-gray-900">
                  {total.toLocaleString()} {service.price.currency.toUpperCase()}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            {errors.acceptTerms && (
              <p className="text-12 text-red-500 ml-8">{errors.acceptTerms}</p>
            )}
            <p className="text-12 text-gray-600 leading-relaxed">
              If you are not around when the delivery person arrives, they will leave your order at the door. by placing your order, you agree to take full responsibility for it once it&apos;s delivered.
            </p>
          </div>
