...
export function OrderDetailsClient({ orderId }: OrderDetailsClientProps) {
  ...
  // Fetch order reviews
  const { data: orderReviews = [], isLoading: isLoadingReviews } =
    useOrderReviews(
  ...
  const productPurchases = purchases.filter(
    (p: PurchaseResponse) => p.type === PurchaseType.Product
  )
  // Filter service purchases - items that are services
  const servicePurchases = purchases.filter(
    (p: PurchaseResponse) => p.type === PurchaseType.Service
  )
  // Total item count - count of all items in the order
  const totalItemCount = purchases.reduce(
  ...
  const shippingAmount =
    order.shippingAmount || order.orderSummary?.shipping || 0
  // Discount amount - total discount applied to the order
  const discountAmount =
    order.discountAmount || order.orderSummary?.discount || 0
              ...
              <div className="text-14 text-gray-600 mb-4 p-3 bg-gray-50 rounded-lg">
                ...
                {deliveryDeliveryDate && (
                  <p className="text-12 mb-1">
                    Delivery: {deliveryDeliveryDate}
                  </p>
                )}
                {deliveryObjectStatus && (
                  <div className="mt-2">
                    <p className="text-12 font-medium text-gray-900 mb-1">
                      Status:
                    </p>
                    <DeliveryStatusBadge
                      status={deliveryObjectStatus}
                      className="text-12"
                    />
                  </div>
                )}
              ...
              <>
                ...
                <div className="space-y-3 mb-4">
                  {productPurchases.map((purchase: PurchaseResponse) => {
                    ...
                    const displayName =
                      ...
                      'Product'
                    const displayImage =
                      product?.image || purchase.imageUrl || ''
                  ...
                  {servicePurchases.map((purchase: PurchaseResponse) => {
                    ...
                    const displayName =
                      ...
                      'Service'
                    const displayImage =
                      service?.imageUrl || purchase.imageUrl || ''
                ...
                {(paymentMethod || paymentStatus || paymentStatusText) && (
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <h5 className="text-14 font-semibold text-gray-900 mb-2">
                      Payment Information
                    </h5>
                    {paymentMethod && (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    {review.rate && (
                      <RatingDisplay rating={review.rate} size="sm" />
                    )}
                    {review.title && (
                ...
                >
                  ...
                  {itemReview.showForm && productId && (
                    <div className="mt-4 p-4 bg-white rounded-lg border border-gray-300">
                      <h5 className="text-14 font-semibold text-gray-900 mb-4">
                        Review This Product
                      </h5>
