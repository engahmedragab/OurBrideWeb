...
export function ProviderStoreClient({ providerId }: ProviderStoreClientProps) {
  ...
  const {
    ...
    error: storeError,
  } = useProviderPublicStore(parseInt(providerId), { page: 1, pageSize: 1000 })
  ...
  const availableCategories =
    productsHomeData?.categories || storeData?.providerCategories || []
  const availableBrands =
    productsHomeData?.brands || storeData?.providerProductBrands || []
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (selectedBrand) {
      productsList = productsList.filter(p => {
        ...
        return (
          storeData?.providerProductBrands?.some(
    ...
    if (selectedTag) {
      productsList = productsList.filter(
        p =>
          ...
          p.providerProductTags?.some(
            pt => pt.productTagId.toString() === selectedTag
          ) ||
          false
      )
    }
    ...
    if (selectedAttribute) {
      productsList = productsList.filter(
        p =>
          ...
          p.providerProductAttributes?.some(
            pa => pa.productAttributeId.toString() === selectedAttribute
          ) ||
          false
      )
    }
    ...
    switch (sortOption) {
      ...
      case 'name':
        productsList.sort((a, b) =>
          (a.nameEn || a.nameAr || '').localeCompare(b.nameEn || b.nameAr || '')
        )
        break
      default:
        // Keep original order
        ...
        {storeData?.providerPublicBannerImageUrl && (
          <div className="relative h-48 md:h-64 lg:h-80 overflow-hidden bg-gradient-to-r from-brand-100 to-purple-100">
            <Image
              ...
              alt={
                storeData?.providerNameEn ||
                storeData?.providerNameAr ||
                'Store Banner'
              }
              fill
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                  ...
                  <p className="text-16 text-gray-600 max-w-2xl">
                    {storeData?.providerDescriptionEn ||
                      storeData?.providerDescriptionAr ||
                      ''}
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center gap-6 mt-4 pt-4 border-t border-gray-200">
              ...
              {(storeData?.totalProductsOnSale || 0) > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-14 font-semibold text-red-600">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {productsHomeData.statistics.customers && (
                  <div className="bg-white rounded-xl p-5 text-center shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                    ...
                    <div className="text-14 text-gray-600 font-medium">
                      Happy Customers
                    </div>
                  </div>
                )}
            ...
            <div className="container-custom py-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-14 text-white/90">
                    Limited time offers - Don't miss out!
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {Object.entries(flashSales)
                  .slice(0, 4)
                  .map(([date, products]) => (
                    <div
                      key={date}
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-6 p-4">
                <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    {/* Sort */}
                    <div className="flex items-center gap-2">
                      <span className="text-14 text-gray-600 whitespace-nowrap">
                        Sort:
                      </span>
                      <Select
                        value={sortOption}
