...
export function ProviderLinksClient({ providerId }: ProviderLinksClientProps) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useProviderLinkee(parseInt(providerId))
  ...
  const provider = {
    ...
    location:
      linkeeData.shortAddress ||
      linkeeData.address?.addressEn ||
      linkeeData.address?.addressAr ||
      '',
  ...
  const buildLinkItems = (): LinkItem[] => {
    ...
    provider.links
      ...
      .forEach((link: LinkResponse) => {
        ...
        if (
          url &&
          (url.includes('instagram.com') || linkType.includes('instagram'))
        ) {
          icon = <Instagram className="h-5 w-5" />
          label = 'Instagram'
        } else if (
          url &&
          (url.includes('facebook.com') || linkType.includes('facebook'))
        ) {
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-md mx-auto px-4 py-6">
          ...
          <div className="flex flex-col items-center mb-4">
            ...
            {/* Provider Name */}
            <h1 className="text-24 font-bold text-gray-900 mb-1 text-center">
              {provider.name}
            </h1>
        ...
        <div className="pt-6 space-y-3">
          ...
          <button
            onClick={() => {
              ...
              if (
                window.confirm('Are you sure you want to report this provider?')
              ) {
