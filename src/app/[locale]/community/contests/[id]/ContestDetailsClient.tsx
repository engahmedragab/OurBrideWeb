...
import { ContestDetails, CommunitySidebar, CommunityRightSidebar, type CommunityTab } from '@/components/community'
import { useCommunityHome } from '@/hooks/home/useHome'
import { getContestById, getContestBySlug } from '@/services/api/contestsApi'
...
export function ContestDetailsClient({ id }: { id: string }) {
  const t = useI18nTranslations("community")
  const activeTab: CommunityTab = 'contests'
  ...
  // Fetch contest data - support both ID and slug
    ...
    return (
      <div className="min-h-screen flex flex-col bg-gray-50">
        ...
        <main className="flex-1 flex items-center justify-center min-h-[60vh] py-12">
          <LoadingOverlay open={true} title={t("contestDetails.loading")} />
        </main>
        <Footer />
      </div>
    )
        ...
        <main className="flex-1 flex items-center justify-center min-h-[60vh] py-12">
          <div className="w-full max-w-md mx-auto px-4">
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8 sm:p-10 text-center">
              <p className="text-gray-600 text-base sm:text-lg font-medium">{t("contestDetails.contestNotFoundTitle")}</p>
              <p className="text-gray-500 text-sm mt-2">{t("contestDetails.contestNotFoundMessage")}</p>
            </div>
          </div>
        </main>
        <Footer />
          ...
          <div className={cn('flex flex-col gap-6 sm:gap-8 lg:gap-10 xl:gap-12 lg:flex-row lg:items-start')}>
            ...
            <article className="flex-1 min-w-0 w-full lg:max-w-4xl xl:max-w-5xl 2xl:max-w-6xl mx-auto lg:mx-0">
              <div className="w-full">
                <ContestDetails contest={contest as LeaderboardContestResponse} />
              </div>
            </article>
