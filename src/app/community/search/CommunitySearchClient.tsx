...
export function CommunitySearchClient() {
  ...
  const [filters, setFilters] = useState({
    ...
    preparationId: initialPreparationId
      ? parseInt(initialPreparationId, 10)
      : undefined,
    providerId: initialProviderId ? parseInt(initialProviderId, 10) : undefined,
    bazaarEventId: initialBazaarEventId
      ? parseInt(initialBazaarEventId, 10)
      : undefined,
  ...
  const updateSearchParams = () => {
    ...
    if (filters.preparationId)
      params.set('preparationId', String(filters.preparationId))
    if (filters.providerId) params.set('providerId', String(filters.providerId))
    if (filters.bazaarEventId)
      params.set('bazaarEventId', String(filters.bazaarEventId))
  ...
  const filteredResults = activeContentType
    ? searchResults?.filter(
        item =>
          item.contentType.toLowerCase() === activeContentType.toLowerCase()
      )
    : searchResults
  ...
  const contentTypeCounts = contentTypes.reduce(
    (acc, type) => {
      acc[type] =
        searchResults?.filter(
          (item: UnifiedCommunityContentResponse) =>
  ...
  const renderContentCard = (content: UnifiedCommunityContentResponse) => {
    switch (content.contentType.toLowerCase()) {
      case 'post':
        return content.post ? (
          <PostCard key={`post-${content.id}`} post={content.post} />
        ) : null
      case 'blog':
        return content.blog ? (
          <BlogCard key={`blog-${content.id}`} blog={content.blog} />
        ) : null
      case 'article':
      ...
      case 'reel':
        return content.reel ? (
          <ReelCard key={`reel-${content.id}`} reel={content.reel} />
        ) : null
      case 'decisiongroup':
        return content.decisionGroup ? (
          <DecisionGroupCard
            key={`decision-group-${content.id}`}
        ...
        <div className="w-full max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 py-6 md:py-8">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="space-y-4">
              <h1 className="text-24 font-normal text-gray-900">
                Search Community
              </h1>
                  ...
                  {contentTypes.map(type => {
                    ...
                    return (
                      <Button
                        key={type}
                        variant={
                          activeContentType === type ? 'brand' : 'outline'
                        }
                        size="sm"
              ...
              {searchQuery && !hasFilters && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-4">
                  <p className="text-14 text-yellow-800">
                    <strong>Note:</strong> Text search is not currently
                    supported by the API. The search query "{searchQuery}"
                    cannot be used. Please use the filters above to find content
                    by Category ID, Item ID, Provider ID, or Tag IDs.
                  </p>
                </div>
              )}
              {!hasFilters && !searchQuery ? (
                <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                  ...
                  <p className="text-14 text-gray-600 mb-4">
                    Use filters to find community content by Category, Item,
                    Provider, or Tags
                  </p>
                </div>
              ) : isLoading ? (
