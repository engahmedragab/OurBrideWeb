...
export function BookingPageClient({ serviceId }: BookingPageClientProps) {
  ...
  const packages: Package[] = useMemo(() => {
    if (
      ...
      rawServiceResponse.packages.length === 0
    ) {
      return []
    }
  ...
  const branches: Branch[] = useMemo(() => {
    if (
      !rawServiceResponse?.servicePlaceAssignments ||
      rawServiceResponse.servicePlaceAssignments.length === 0
    ) {
      return []
    }
    ...
    return rawServiceResponse.servicePlaceAssignments
      .filter(
        (assignment: ServicePlaceAssignmentResponse) =>
          assignment.place !== null
      )
  ...
  const staff: Staff[] = useMemo(() => {
    if (
      !rawServiceResponse?.serviceStaffAssignments ||
      rawServiceResponse.serviceStaffAssignments.length === 0
    ) {
      return []
    }

    return rawServiceResponse.serviceStaffAssignments.map(
      (assignment: ServiceStaffAssignmentResponse) => ({
  ...
  useEffect(() => {
    ...
    const fetchTimeSlots = async () => {
      ...
      try {
        ...
        const mappedSlots: TimeSlot[] = slots.map(
          (slot: TimeSlotResponse, index: number) => {
  ...
  const serviceDuration = useMemo(() => {
    ...
    if (selectedPackageData?.description) {
      ...
      const durationMatch =
        selectedPackageData.description.match(/(\d+)\s*min/i)
      if (durationMatch) {
  ...
  const handleReservationStatusUpdate = (
    ...
    if (!finalReservationId) {
      ...
      setIsSubmitting(false)
      alert(
        'Reservation found but missing ID. Please check your reservations page.'
      )
      return
    }
    ...
    switch (status) {
      ...
      case ReservationStatus.Rejected:
        ...
        setIsSubmitting(false)
        alert(
          'Reservation was rejected. Please check your reservations page for details.'
        )
        break
  ...
  const createPurchaseAndNavigate = async (
    ...
    try {
      ...
      const servicePrice =
        service?.price?.original || service?.price?.discounted || 0
      ...
      const purchasePayload: PurchaseRequest = {
        ...
        serviceType:
          (rawServiceResponse?.type as ServiceType) || ServiceType.Rent,
        ...
        endDate: requestedStartTimeParam
          ? new Date(
              new Date(requestedStartTimeParam).getTime() + 60 * 60 * 1000
            ).toISOString()
      ...
      console.log(
        '🟡 Purchase creation failed, but navigating to reservation details anyway...'
      )
      router.push(`/reservations/${reservationId}`)
    }
  }
  ...
  const handlePollingTimeout = () => {
    ...
    setIsSubmitting(false)
    alert(
      'Reservation is taking longer than expected. Please check your reservations page.'
    )
  }
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (selectedSlot) {
        ...
        reservationDate = new Date(selectedSlot.start)
          .toISOString()
          .split('T')[0]
        ...
        if (dateMatch) {
          ...
          if (monthIndex !== -1) {
            ...
            const timeMatch = formData.selectedTime.match(
              /(\d+):(\d+)\s*(AM|PM)?/i
            )
            if (timeMatch) {
      ...
      const reservationRequest: ReservationRequest = {
        ...
        branchId: formData.selectedBranch
          ? parseInt(formData.selectedBranch, 10)
          : undefined,
        staffId: formData.selectedStaff
          ? parseInt(formData.selectedStaff, 10) || undefined
          : undefined,
        servicePackageId: formData.selectedPackage
          ? parseInt(formData.selectedPackage, 10)
          : undefined,
      ...
      const notes = formData.promoCode
      ...
      try {
        ...
        const responseData = apiResponse as unknown as {
          reservationId?: string
          id?: string | number
          status?: ReservationStatus
        }
        const reservationId =
          apiResponse?.reservationId ||
        ...
        if (!reservationId) {
          ...
          throw new Error(
            'Reservation ID not found in response. Please try again.'
          )
        }
        ...
        if (errorStatus === 202 || errorStatus === 201) {
          const reservationId =
            err.response?.data?.reservationId ||
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  {staffMember.role && (
                                    <div className="text-12 text-gray-500 truncate">
                    ...
                    <div className="bg-brand-50 border border-brand-200 rounded-xl p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-14 text-gray-600">
                            Your appointment will be confirmed and you&apos;ll
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <Button
                  ...
                  disabled={
                    ...
                    (step === 'confirm' &&
                      (!formData.selectedTime || !formData.acceptTerms))
                  }
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-8 space-y-6">
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-200">
                  ...
                  <p className="text-12 text-gray-500 mb-4">
                    1 service selected
                  </p>
                      ...
                      <div className="space-y-2">
                        ...
                        {formData.selectedBranch && (
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-12 text-gray-900 font-medium">
                              {branches.find(
                                b => b.id === formData.selectedBranch
                              )?.name || 'N/A'}
                            </span>
                          </div>
                        )}
                        ...
                        {formData.selectedStaff && (
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-12 text-gray-900 font-medium">
                              {staff.find(s => s.id === formData.selectedStaff)
                                ?.name || 'N/A'}
                            </span>
                          </div>
                        )}
                            ...
                            return (
                              <div className="flex justify-between items-center">
                                ...
                                <div className="text-right">
                                  {formattedDate && (
                                    <span className="text-12 text-gray-900 font-medium block">
                                      {formattedDate}
                                    </span>
                                  )}
                                  {slotTime && (
                ...
                {step === 'confirm' && (
                  <div className="bg-white rounded-xl p-6 border border-gray-200">
                    ...
                    <p className="text-12 text-gray-600 leading-relaxed ml-8">
                      If you are not around when the delivery person arrives,
                      they will leave your order at the door. By placing your
