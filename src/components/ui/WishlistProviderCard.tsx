...
export const WishlistProviderCard = React.memo(
  ({
    ...
    const providerName =
      provider.nameEn || provider.nameAr || `Provider #${provider.id}`
    const providerImage =
      provider.publicLogoImageUrl || provider.publicBannerImageUrl || ''

    const handleViewProfile = () => {
      const profileUrl =
        provider.publicProfileSlug || `/providers/${provider.id}`
    ...
    return (
      ...
      >
        <div className="flex flex-col">
          {/* Image Section (Top) */}
          <Link
            href={provider.publicProfileSlug || `/providers/${provider.id}`}
            ...
            <div className="space-y-3 flex-1">
              ...
              {provider.shortAddress && (
                <div className="flex items-start gap-2">
                  ...
                  <span className="text-13 text-gray-600 line-clamp-1">
                    {provider.shortAddress}
                  </span>
                </div>
              )}
