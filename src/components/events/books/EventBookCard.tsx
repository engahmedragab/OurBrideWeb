/**
 * Event Book Card Component
 * Displays event book with its lines
 */

import { cn } from '@/lib/utils'
import type { MainEventBookResponse } from '@/types/responses'

interface EventBookCardProps {
  book: MainEventBookResponse
  onInit?: () => Promise<void>
  onNavigate?: () => void
  eventId?: number
}

export function EventBookCard({ book, onInit, onNavigate }: EventBookCardProps) {
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-14 text-gray-500">{activeLines.length} events</span>
        </div>
      </div>
      ...
      {activeLines.length > 0 ? (
        ...
        <p className="text-14 text-gray-500 text-center py-4">No events yet</p>
      )}
    </div>
  )
}
