  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
      ...
      <p className={`${planningTypography.secondary} mb-3`}>
       {t('summaryCard.completedOfTotal', { completed, total })}
      </p>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="text-center">
          ...
          <p className={`${planningTypography.statLabel} mt-0.5`}>{t('summaryCard.total')}</p>
        </div>
        <div className="text-center">
          <p className={`${planningTypography.statNumber} text-green-500`}>{completed}</p>
          <p className={`${planningTypography.statLabel} mt-0.5`}>{t('summaryCard.completed')}</p>
        </div>
        <div className="text-center">
          <p className={`${planningTypography.statNumber} text-yellow-600`}>{inProgress}</p>
          <p className={`${planningTypography.statLabel} mt-0.5`}>{t('summaryCard.inProgress')}</p>
        </div>
      </div>
    </div>
  )
